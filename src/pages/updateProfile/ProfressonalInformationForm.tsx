...
const ProfressonalInformationForm = ({
  ...
  useEffect(() => {
    setValue(
      'occupation',
      data?.data?.profile[data?.data?.profile?.length - 1]?.ocupation
    )

    setProfessonalInfo({
      ...professonalInfo,
      description:
        data?.data?.profile[data?.data?.profile?.length - 1]?.description,
      experience: {
        name: data?.data?.profile[data?.data?.profile?.length - 1]?.experience,
      },
      education: [
        {
          major: data?.data?.profile[data?.data?.profile?.length - 1]?.major,
          school:
            data?.data?.profile[data?.data?.profile?.length - 1]?.education,
        },
      ],
      certificate: [
        {
          cow: data?.data?.profile[data?.data?.profile?.length - 1]
            ?.certificate,
          from: data?.data?.profile[data?.data?.profile?.length - 1]
            ?.certificateOrigin,
        },
      ],
    })
          ...
          (item: number, index: number) => (
            <div className='grid grid-cols-delete gap-3' key={item}>
              ...
              <Input
                ...
                suffix={<URLIcon className='absolute top-7 right-3' />}
              />
              <div className='flex items-center justify-end pr-2 cursor-pointer'>
                <DeleteIcon
                  onClick={() => {
